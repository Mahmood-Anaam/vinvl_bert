...
class VinVLBertPipeline:
    """
    Pipeline for generating image captions using the VinVLBertPipeline BiT Image Captioning model.
    """

    def __init__(self, cfg=VinVLBertConfig):
        """
        Initializes the pipeline with the specified configuration.

        Args:
            cfg (BerTConfig): Configuration object containing all settings for the pipeline.
        """

        self.cfg = cfg
        # dowbload the model from huggingface
        checkpoint = snapshot_download(repo_id=cfg.model_id,repo_type="model")
        ...
        self.feature_extractor = VinVLFeatureExtractor(device=self.cfg.device)
